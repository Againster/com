...
*/
void MD5::Update(const void *input, std::streamsize length) {
...
** context.
*/
void MD5::Update(const unsigned char *input, std::streamsize length) {
	...
	memcpy(&_buffer[index], &input[i], static_cast<size_t>(length - i));
}
...
*/
string MD5::UCsToHexString(const unsigned char *input, size_t length) {
	string str;
...
*/
string MD5::ToString() {
	return UCsToHexString(Digest(), 16);
}
